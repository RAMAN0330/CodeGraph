function App(){
    ...
    async function readLocalFolder(dirHandle, compiledPatterns, path=''){
        ...
        async function finishAnalysis(){
            ...
            var highComplexity=analyzed.filter(function(f){return f.complexity&&f.complexity.level==='critical';}).sort(function(a,b){return b.complexity.score-a.complexity.score;});
            if(highComplexity.length)issues.push({type:'warning',title:highComplexity.length+' High Complexity Files',desc:'Files with complexity score >30',items:highComplexity.map(function(f){return{name:f.name+' ('+f.complexity.score+')',file:f.path,score:f.complexity.score,lines:f.lines};})});
            var dataObj={files:analyzed,functions:allFns,connections:conns,fnStats:fnStats,folders:folders,tree:tree,issues:issues,patterns:patterns,securityIssues:securityIssues,duplicates:duplicates,layerViolations:layerViolations,deadFunctions:deadFns.map(function(x){var codeLines=x[1].code?x[1].code.split('\n').length:0;return{name:x[0],file:x[1].file,folder:x[1].folder,line:x[1].line,code:x[1].code,codeLines:codeLines,ext:x[1].file.split('.').pop()};}),excludePatterns:(compiledPatterns||[]).map(function(x){return x.raw;}),stats:{files:analyzed.length,functions:allFns.length,connections:conns.length,dead:deadFns.length,patterns:patterns.length,security:securityIssues.filter(function(i){return i.severity==='high';}).length,duplicates:duplicates.length,violations:layerViolations.length,loc:totalLoc,languages:langArray}};
            dataObj.suggestions=Parser.generateSuggestions(dataObj);
            setData(dataObj);
            setExpandedPaths(new Set(['']));
    ...
    useEffect(function(){
        ...
        var zoom=d3.zoom().scaleExtent([0.05,5]).on('zoom',function(e){container.attr('transform',e.transform);});
        svg.call(zoom);
        ...
        if(graphConfig.viewMode==='force'){
            ...
            sim.force('link',d3.forceLink(links).id(function(d){return d.id;}).distance(linkDist).strength(0.3))
               .force('charge',d3.forceManyBody().strength(-spacing).distanceMax(400*breathe))
               .force('collision',d3.forceCollide().radius(function(d){return getR(d)+collidePad;}))
            ...
            sim.force('link',d3.forceLink(links).id(function(d){return d.id;}).distance(linkDist*0.5).strength(0.05))
               .force('charge',d3.forceManyBody().strength(-spacing*0.3))
               .force('collision',d3.forceCollide().radius(function(d){return getR(d)+collidePad*0.7;}))
            ...
            sortedLayers.forEach(function(l,li){var g=layerGroups[l];var colW=wideW/(sortedLayers.length+1);g.forEach(function(n,ni){n.targetX=(li+1)*colW;n.targetY=(ni+1)*Math.max(h,h*breathe*0.7)/(g.length+1);});});
            sim.force('link',d3.forceLink(links).id(function(d){return d.id;}).distance(linkDist).strength(0.1))
               .force('charge',d3.forceManyBody().strength(-spacing*0.5).distanceMax(200*breathe))
               .force('collision',d3.forceCollide().radius(function(d){return getR(d)+collidePad*0.85;}))
            ...
            sim.force('link',d3.forceLink(links).id(function(d){return d.id;}).distance(linkDist*1.5).strength(0.02))
               .force('collision',d3.forceCollide().radius(function(d){return getR(d)+collidePad*1.25;}))
            ...
            sim.force('link',d3.forceLink(links).id(function(d){return d.id;}).distance(linkDist).strength(0.05))
               .force('collision',d3.forceCollide().radius(function(d){return getR(d)+collidePad;}))
    ...
    useEffect(function(){
        ...
        var g=svg.append('g');
        var zoom=d3.zoom().scaleExtent([0.2,6]).on('zoom',function(e){g.attr('transform',e.transform);});
        svg.call(zoom);
        var filteredFiles=folderFilter?data.files.filter(function(f){return f.folder===folderFilter||f.folder.startsWith(folderFilter+'/');}):data.files;
        ...
        var root=d3.hierarchy(rootNode).sum(function(d){return d.value||0;}).sort(function(a,b){return b.value-a.value;});
        d3.treemap().size([w-8,h-8]).paddingOuter(3).paddingTop(function(d){return d.depth===0?0:16;}).paddingInner(2).round(true)(root);
        ...
        parentG.append('rect')
            ...
            .attr('fill',function(d){var top=d.ancestors().find(function(a){return a.depth===1;});return colorMap[top?top.data.fullPath:d.data.fullPath]||COLORS[parents.indexOf(d)%COLORS.length];})
            .attr('fill-opacity',0.08).attr('stroke',function(d){var top=d.ancestors().find(function(a){return a.depth===1;});return colorMap[top?top.data.fullPath:d.data.fullPath]||COLORS[parents.indexOf(d)%COLORS.length];})
            .attr('stroke-opacity',0.45).attr('rx',5).style('cursor','pointer');
        parentG.filter(function(d){return d.x1-d.x0>40&&d.y1-d.y0>18;}).append('text')
            ...
            .text(function(d){var maxLen=Math.floor((d.x1-d.x0-10)/6);var n=d.data.name;return n.length>maxLen?n.slice(0,maxLen-1)+'…':n;});
        parentG.on('click',function(e,d){e.stopPropagation();if(d.data.fullPath)filterByFolder(d.data.fullPath);});

        // Leaves (files)
        var leafColor=function(d){var top=d.ancestors().find(function(a){return a.depth===1;});return colorMap[top?top.data.fullPath:d.data.folder]||COLORS[0];};
            ...
            if(d.data.path&&selectFileRef.current){
                ...
                setTimeout(function(){
                    ...
                    cells.select('rect').transition().duration(300)
                        ...
                        .attr('fill',function(n){return n.data.path===d.data.path?'#ff5f5f':(blast&&blast.affected.includes(n.data.path))?'#ff9f43':leafColor(n);})
    ...
    useEffect(function(){
        ...
        var zoom=d3.zoom().scaleExtent([0.2,4]).on('zoom',function(e){g.attr('transform','translate('+(w/2+e.transform.x)+','+(h/2+e.transform.y)+') scale('+e.transform.k+')');});
        svg.call(zoom);
        ...
        var node=g.selectAll('g.dendro-node').data(root.descendants()).join('g').attr('class','dendro-node')
            .attr('transform',function(d){return'rotate('+(d.x*180/Math.PI-90)+') translate('+d.y+',0)';}).style('cursor','pointer');
        node.append('circle').attr('r',function(d){return d.children?4:6;})
            .attr('fill',function(d){return d.children?'var(--bg3)':colorMap[d.data.folder]||COLORS[0];})
            .attr('stroke',function(d){return d.children?'var(--t3)':'var(--bg0)';}).attr('stroke-width',1.5);

        // Leaf labels: tangent to circle, flipped on left half
        node.filter(function(d){return!d.children;}).append('text')
            .attr('dy','0.31em')
            .attr('x',function(d){return d.x<Math.PI?8:-8;})
            ...
            .text(function(d){var n=d.data.name.replace(/\.[^.]+$/,'');return n.length>22?n.slice(0,20)+'…':n;});

        // Folder labels
        node.filter(function(d){return d.children&&d.depth>0;}).append('text')
            .attr('dy','0.31em')
            .attr('x',function(d){return d.x<Math.PI?-8:8;})
        ...
        node.on('mouseenter',function(e,d){
            ...
            d3.select(this).select('circle').transition().duration(150).attr('r',function(n){return n.children?7:10;}).attr('stroke','var(--acc)').attr('stroke-width',2.5);
        }).on('mousemove',function(e){tooltip.style('left',(e.offsetX+15)+'px').style('top',(e.offsetY+15)+'px');})
        .on('mouseleave',function(e,d){
            tooltip.style('display','none');
    ...
    useEffect(function(){
        ...
        var zoom=d3.zoom().scaleExtent([0.2,3]).on('zoom',function(e){gRoot.attr('transform',e.transform);});
        svg.call(zoom);
        ...
        var fileCountByFolder={};filteredFiles.forEach(function(f){var k=f.folder||'root';fileCountByFolder[k]=(fileCountByFolder[k]||0)+1;});
        ...
        data.connections.forEach(function(c){
            ...
            var sf=data.files.find(function(f){return f.path===src;});
            var tf=data.files.find(function(f){return f.path===tgt;});
            if(!sf||!tf)return;
        ...
        var nodes=folders.map(function(f){return{id:f,name:f.split('/').pop()||'root',fullPath:f,fileCount:fileCountByFolder[f]||0,layer:0};});
        var nodeById={};nodes.forEach(function(n){nodeById[n.id]=n;});
        var edges=Object.keys(edgeMap).map(function(k){var p=k.split(' ');return{source:p[0],target:p[1],value:edgeMap[k]};});
        ...
        edges.forEach(function(e){adj[e.source].push(e.target);indeg[e.target]++;});
        ...
        var maxLayer=Math.max(0,d3.max(nodes,function(n){return n.layer;})||0);
        var layers=[];for(var i=0;i<=maxLayer;i++)layers.push([]);
        nodes.forEach(function(n){layers[n.layer].push(n);});
        ...
        layers.forEach(function(col,li){
            ...
            col.sort(function(a,b){return b.fileCount-a.fileCount;});
        ...
        // Edge layer
        var maxVal=d3.max(edges,function(e){return e.value;})||1;
        ...
        var edgePaths=edgeG.selectAll('path').data(edges).join('path')
            ...
            .attr('fill','none')
            .attr('stroke',function(e){return colorMap[e.source]||COLORS[folderIdx[e.source]%COLORS.length];})
            .attr('stroke-width',function(e){return Math.max(1.5,Math.min(6,1+Math.log2(e.value+1)*1.4));})
        ...
        nodeSel.append('rect')
            ...
            .attr('rx',10)
            .attr('fill',function(n){return colorMap[n.id]||COLORS[folderIdx[n.id]%COLORS.length];})
            .attr('fill-opacity',0.15)
            .attr('stroke',function(n){return colorMap[n.id]||COLORS[folderIdx[n.id]%COLORS.length];})
        ...
        nodeSel.append('text').attr('text-anchor','middle').attr('dy',-2)
            ...
            .text(function(n){var nm=n.name;return nm.length>18?nm.slice(0,17)+'…':nm;});
        nodeSel.append('text').attr('text-anchor','middle').attr('dy',12)
            ...
            .text(function(n){return n.fileCount+' file'+(n.fileCount===1?'':'s');});

        nodeSel.on('mouseenter',function(e,d){
            tooltip.html(renderTooltipHtml(d.fullPath,[{label:'Files',value:d.fileCount}]))
                .style('display','block').style('left',(e.offsetX+15)+'px').style('top',(e.offsetY+15)+'px');
            edgePaths.attr('stroke-opacity',function(l){return l.source===d.id||l.target===d.id?0.9:0.08;});
            flowAnim.attr('stroke-opacity',function(l){return l.source===d.id||l.target===d.id?1:0;});
        }).on('mouseleave',function(){
            tooltip.style('display','none');
        ...
        edgePaths.on('mouseenter',function(e,d){
    ...
    function exportJSON(){if(!data)return;var blob=new Blob([JSON.stringify({stats:data.stats,files:data.files.map(function(f){return{path:f.path,fns:f.functions.length,layer:f.layer,lines:f.lines,dependencies:f.dependencies||[]};}),connections:data.connections,issues:data.issues,patterns:data.patterns,security:data.securityIssues},null,2)],{type:'application/json'});var url=URL.createObjectURL(blob);var a=document.createElement('a');a.href=url;a.download='codegraph-analysis.json';a.click();}
    ...
    return React.createElement('div',{className:'app'},
        React.createElement('div',{className:'topbar'},
            ...
            React.createElement('div',{className:'repo-input-group'},
                ...
                React.createElement('select',{className:'auth-select','aria-label':'Authentication Method',title:'Authentication method. Use PAT or GitHub App for private repos.',value:authMethod,onChange:function(e){setAuthMethod(e.target.value);}},
                ...
                React.createElement('div',{className:'auth-inputs'},
                    authMethod==='pat'&&React.createElement(React.Fragment,null,
                        React.createElement('input',{className:'repo-input',type:'password','aria-label':'GitHub Token',placeholder:'ghp_… Personal Access Token',value:token,onChange:function(e){setToken(e.target.value);},onKeyDown:function(e){if(e.key==='Enter'&&!loading)analyze();},style:{minWidth:180}}),
            ...
            React.createElement('div',{className:'topbar-group'},
                ...
                React.createElement('button',{className:'top-btn','aria-label':'Toggle theme',onClick:function(){setTheme(function(t){return t==='dark'?'light':'dark';});}},React.createElement(Icon,{name:theme==='dark'?'sun':'moon',size:'m'}),theme==='dark'?'Light':'Dark')
            )
        ),
