export default function WorkspaceArea(){
            ...
            function beginRepoAnalysis(){
                ...
                function processFile(i){
                    ...
                    if(isCodeFile){
                        ...
                        GitHub.getFile(p.owner,p.repo,f.path).then(function(content: any){
                            var layer=Parser.detectLayer(f.path);
    ...
    function openFilePreview(path,line){
        ...
        if(localDirHandle){
            ...
            // Fetch from GitHub
            GitHub.getFile(repoInfo.owner,repoInfo.repo,path).then(function(content: any){
                if(content){
                    setFilePreview({path:path,filename:filename,content:content,line:line||null,loading:false,error:null});
                }else{
    ...
    useEffect(function(){
        ...
        parentG.append('rect')
            ...
            .attr('fill',function(d: any){var top=d.ancestors().find(function(a: any){return a.depth===1;});return colorMap[top?top.(data as any).fullPath:d.(data as any).fullPath]||COLORS[parents.indexOf(d)%COLORS.length];})
            .attr('fill-opacity',0.08).attr('stroke',function(d: any){var top=d.ancestors().find(function(a: any){return a.depth===1;});return colorMap[top?top.(data as any).fullPath:d.(data as any).fullPath]||COLORS[parents.indexOf(d)%COLORS.length];})
        ...
        parentG.filter(function(d: any){return d.x1-d.x0>40&&d.y1-d.y0>18;}).append('text')
            ...
            .text(function(d: any){var maxLen=Math.floor((d.x1-d.x0-10)/6);var n=d.(data as any).name;return n.length>maxLen?n.slice(0,maxLen-1)+'…':n;});
        parentG.on('click',function(e: any, d: any){e.stopPropagation();if(d.(data as any).fullPath)filterByFolder(d.(data as any).fullPath);});

        // Leaves (files)
        var leafColor=function(d: any){var top=d.ancestors().find(function(a: any){return a.depth===1;});return colorMap[top?top.(data as any).fullPath:d.(data as any).folder]||COLORS[0];};
        ...
        cells.filter(function(d: any){return d.x1-d.x0>45&&d.y1-d.y0>22;}).append('text').attr('class','treemap-text')
            ...
            .text(function(d: any){var n=d.(data as any).name.replace(/\.[^.]+$/,'');var maxLen=Math.floor((d.x1-d.x0-8)/6);return n.length>maxLen?n.slice(0,maxLen-1)+'…':n;});
        cells.filter(function(d: any){return d.x1-d.x0>60&&d.y1-d.y0>35;}).append('text').attr('class','treemap-subtext')
            ...
            .text(function(d: any){return d.(data as any).value+' lines';});
        ...
        cells.on('mouseenter',function(e: any, d: any){
            tooltip.html(renderTooltipHtml(d.(data as any).name,[
                {label:'Lines',value:d.(data as any).value},
                {label:'Functions',value:d.(data as any).fns||0},
                {label:'Layer',value:d.(data as any).layer||'—'},
                {label:'Folder',value:d.(data as any).folder||'root'}
        ...
        .on('mouseleave',function(e: any, d: any){
            ...
            var isSelected=d.(data as any).path===sel;
            var isAffected=blastRadius&&blastRadius.affected.includes(d.(data as any).path);
            ...
            e.stopPropagation();
            if(d.(data as any).path&&selectFileRef.current){
                selectFileRef.current(d.(data as any).path);
                setTimeout(function(){
                    var blast=blastRadius;
                    cells.select('rect').transition().duration(300)
                        .attr('opacity',function(n: any){return n.(data as any).path===d.(data as any).path?1:(blast&&blast.affected.includes(n.(data as any).path))?0.95:0.4;})
                        .attr('fill',function(n: any){return n.(data as any).path===d.(data as any).path?'#ff5f5f':(blast&&blast.affected.includes(n.(data as any).path))?'#ff9f43':leafColor(n);})
                        .attr('stroke',function(n: any){return n.(data as any).path===d.(data as any).path?'#ff5f5f':(blast&&blast.affected.includes(n.(data as any).path))?'var(--orange)':'var(--bg0)';})
                        .attr('stroke-width',function(n: any){return n.(data as any).path===d.(data as any).path||blast&&blast.affected.includes(n.(data as any).path)?2:1;});
                },100);
            }
        });
    ...
    useEffect(function(){
        ...
        node.append('circle').attr('r',function(d: any){return d.children?4:6;})
            .attr('fill',function(d: any){return d.children?'var(--bg3)':colorMap[d.(data as any).folder]||COLORS[0];})
        ...
        node.filter(function(d: any){return!d.children;}).append('text')
            ...
            .text(function(d: any){var n=d.(data as any).name.replace(/\.[^.]+$/,'');return n.length>22?n.slice(0,20)+'…':n;});
        ...
        node.filter(function(d: any){return d.children&&d.depth>0;}).append('text')
            ...
            .text(function(d: any){return d.(data as any).name;});

        node.on('mouseenter',function(e: any, d: any){
            tooltip.html(renderTooltipHtml(d.(data as any).name||'root',d.(data as any).path?[
                {label:'Lines',value:d.(data as any).lines||0},
                {label:'Functions',value:d.(data as any).fns||0},
                {label:'Layer',value:d.(data as any).layer||'—'}
        ...
        .on('mouseleave',function(e: any, d: any){
            ...
            e.stopPropagation();
            if(d.(data as any).path&&selectFileRef.current)selectFileRef.current(d.(data as any).path);
            else if(d.(data as any).fullPath)filterByFolder(d.(data as any).fullPath);
        });
        ...
        React.createElement('div',{className:'topbar'},
            React.createElement('div',{className:'topbar-group'},
                React.createElement('div',{className:'logo',onClick:function(){setShowPrivacy(true);}},
                    ...
                    React.createElement('span',{className:'logo-text'},'CODEGRAPH')
                )
            ),
            React.createElement('div',{className:'topbar-group flex'},
            React.createElement('div',{className:'repo-input-group'},
                React.createElement('input',{className:'repo-input','aria-label':'Repository URL',placeholder:'owner/repo or GitHub URL',value:repoUrl,onChange:function(e: any){setRepoUrl(e.target.value);},onKeyDown:function(e: any){if(e.key==='Enter'&&!loading)analyze();}}),
                React.createElement('select',{className:'auth-select','aria-label':'Authentication Method',title:'Authentication method. Use PAT or GitHub App for private repos.',value:authMethod,onChange:function(e: any){setAuthMethod(e.target.value);}},
                    React.createElement('option',{value:'none'},'Public Repo'),
                    React.createElement('option',{value:'pat'},'Private Repo (PAT)'),
                    React.createElement('option',{value:'github_app'},'GitHub App')
                ),
                React.createElement('div',{className:'auth-inputs'},
                    authMethod==='pat'&&React.createElement(React.Fragment,null,
                        React.createElement('input',{className:'repo-input',type:'password','aria-label':'GitHub Token',placeholder:'ghp_… Personal Access Token',value:token,onChange:function(e: any){setToken(e.target.value);},onKeyDown:function(e: any){if(e.key==='Enter'&&!loading)analyze();},style:{minWidth:180}}),
                        React.createElement('a',{className:'auth-help-link',href:'https://github.com/settings/tokens/new?scopes=repo&description=CodeGraph',target:'_blank',rel:'noopener noreferrer',title:'Generate a new PAT on GitHub (opens in new tab)'},
                            React.createElement(Icon,{name:'key',size:'m'}),' Get Token'
                        )
                    ),
                    authMethod==='github_app'&&React.createElement(React.Fragment,null,
                        React.createElement('input',{className:'repo-input','aria-label':'App ID',placeholder:'App ID',value:appId,onChange:function(e: any){setAppId(e.target.value);},onKeyDown:function(e: any){if(e.key==='Enter'&&!loading)analyze();},style:{width:80}}),
                ...
                React.createElement('button',{id:'analyze-btn',className:'top-btn primary','aria-label':'Analyze repository',onClick:analyze,disabled:loading||!repoUrl},
                    React.createElement(Icon,{name:loading?'activity':'search',size:'m'}),
                    'Analyze'
                ),
                React.createElement('button',{className:'top-btn','aria-label':'Open local folder',onClick:function(){openLocalFolder();},disabled:loading},
            ...
            React.createElement('div',{className:'topbar-group'},
                React.createElement('button',{className:'top-btn','aria-label':'Analyze Pull Request',onClick:function(){setShowPR(true);},disabled:!data||localDirHandle},React.createElement(Icon,{name:'pull-request',size:'m'}),'PR'),
                React.createElement('button',{className:'top-btn','aria-label':'Export analysis',onClick:function(){setShowExport(true);},disabled:!data},React.createElement(Icon,{name:'export',size:'m'}),'Export'),
                React.createElement('button',{className:'top-btn','aria-label':'Copy share link',onClick:copyLink,disabled:!data||localDirHandle},React.createElement(Icon,{name:'share',size:'m'}),'Share'),
                React.createElement('button',{className:'top-btn','aria-label':'Toggle theme',onClick:function(){setTheme(function(t: any){return t==='dark'?'light':'dark';});}},React.createElement(Icon,{name:theme==='dark'?'sun':'moon',size:'m'}),theme==='dark'?'Light':'Dark')
            )
        ),
        React.createElement('div',{className:'main',style:{'--sidebar-w':sidebarWidth+'px','--panel-w':rightPanelWidth+'px'}},
            React.createElement('div',{className:'sidebar',style:{width:sidebarWidth}},
                ...
                data?React.createElement(React.Fragment,null,
                    ...
                    React.createElement('div',{className:'sidebar-scroll'},React.createElement(TreeNode,{node:(data as any).tree,selected:selected,onSelect:selectFile,expanded:expandedPaths,toggle:togglePath,filterFolder:filterByFolder,activeFilter:folderFilter}))
                    ...
                    React.createElement('div',{className:'empty-desc'},'Enter a GitHub URL or click "Open Folder" to analyze a local directory')
                )
            ),
            React.createElement('div',{className:'canvas-area'},
                ...
                !data?React.createElement('div',{className:'empty-state'},
                    ...
                    React.createElement('div',{className:'empty-title'},'CodeGraph'),
                    React.createElement('div',{className:'empty-desc'},'Visualize architecture, blast radius, ownership, patterns & security.\nEnter a GitHub URL or open a local folder to get started.'),
                    React.createElement('div',{className:'empty-features'},
                        React.createElement('span',{className:'empty-feature'},'⚡ Dependency Graph'),
                        React.createElement('span',{className:'empty-feature'},'💥 Blast Radius'),
                        React.createElement('span',{className:'empty-feature'},'🔒 Security Scan'),
                        React.createElement('span',{className:'empty-feature'},'🧩 Pattern Detection'),
                        React.createElement('span',{className:'empty-feature'},'👥 Code Ownership'),
                        React.createElement('span',{className:'empty-feature'},'🔑 Private Repos')
                    )
                ):
            ...
            React.createElement('div',{className:'right-panel',style:{width:rightPanelWidth}},
                ...
                data?React.createElement(React.Fragment,null,
                    ...
                    React.createElement('div',{className:'empty-desc'},'Analyze a GitHub repo or local folder to see insights')
                )
            )
        ),
            ...
            React.createElement('div',{className:'modal',onClick:function(e: any){e.stopPropagation();},style:{maxWidth:600,maxHeight:'85vh',display:'flex',flexDirection:'column'}},
                React.createElement('div',{className:'modal-header'},
                    React.createElement('div',{className:'modal-title'},
                        drillDown.type==='issue'?React.createElement(React.Fragment,null,React.createElement(StatusDot,{color:drillDown.(data as any).type==='critical'?'var(--red)':'var(--orange)'}),' ',drillDown.(data as any).title):
                        drillDown.type==='pattern'?iconLabel(drillDown.(data as any).icon,drillDown.(data as any).name,'m'):
                        drillDown.type==='security'?React.createElement(React.Fragment,null,React.createElement(StatusDot,{color:getSeverityColor(drillDown.(data as any).severity)}),' ',drillDown.(data as any).title):
                        drillDown.type==='duplicate'?iconLabel(drillDown.(data as any).type==='code'?'copy':'note',(drillDown.(data as any).type==='code'?'Similar Code':'Duplicate Name')+': '+drillDown.(data as any).name,'m'):
                        'Details'
                    ),
                ...
                React.createElement('div',{className:'modal-body',style:{overflowY:'auto',flex:1}},
                    ...
                    drillDown.type==='issue'&&React.createElement(React.Fragment,null,
                        React.createElement('div',{style:{background:'var(--bg0)',padding:12,borderRadius:8,marginBottom:16}},
                            React.createElement('div',{style:{fontSize:11,color:'var(--t2)'}},drillDown.(data as any).desc)
                        ),
                        React.createElement('div',{style:{fontSize:12,fontWeight:600,marginBottom:12}},'All Affected Items (',drillDown.(data as any).items?drillDown.(data as any).items.length:0,')'),
                        drillDown.(data as any).items&&drillDown.(data as any).items.map(function(item: any, j: any){return React.createElement('div',{key:j,style:getAccentBlockStyle('rgba(0,255,157,0.28)','rgba(0,255,157,0.08)',{padding:12,marginBottom:8})},
                    ...
                    drillDown.type==='pattern'&&React.createElement(React.Fragment,null,
                        React.createElement('div',{style:{background:'var(--bg0)',padding:12,borderRadius:8,marginBottom:16}},
                            React.createElement('div',{style:{fontSize:11,color:'var(--t2)'}},drillDown.(data as any).desc),
                            drillDown.(data as any).isAnti&&React.createElement('div',{style:{marginTop:8}},React.createElement('span',{className:'badge badge-danger'},'Anti-pattern'))
                        ),
                        drillDown.(data as any).metrics&&React.createElement('div',{style:{display:'flex',gap:12,marginBottom:16}},
                            Object.entries(drillDown.(data as any).metrics).map(function(e: any){return React.createElement('div',{key:e[0],style:{background:'var(--bg0)',padding:12,borderRadius:8,textAlign:'center',flex:1}},
                        ...
                        React.createElement('div',{style:{fontSize:12,fontWeight:600,marginBottom:12}},'All Files (',drillDown.(data as any).files.length,')'),
                        drillDown.(data as any).files.map(function(f: any, j: any){return React.createElement('div',{key:j,style:getAccentBlockStyle('rgba(0,255,157,0.28)','rgba(0,255,157,0.08)',{padding:12,marginBottom:8})},
                            React.createElement('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center'}},
                    ...
                    drillDown.type==='security'&&React.createElement(React.Fragment,null,
                        React.createElement('div',{style:drillDown.(data as any).severity==='high'
                            ? getAccentBlockStyle('rgba(255,95,95,0.36)','rgba(255,95,95,0.1)',{padding:12,marginBottom:16})
                            : drillDown.(data as any).severity==='medium'
                            ...
                            React.createElement('div',{style:{fontSize:11,fontWeight:600,marginBottom:4}},drillDown.(data as any).severity.toUpperCase()+' Severity'),
                            React.createElement('div',{style:{fontSize:11,color:'var(--t2)'}},drillDown.(data as any).desc)
                        ),
                        React.createElement('div',{style:{fontSize:12,fontWeight:600,marginBottom:12}},'Location'),
                        React.createElement('div',{style:{background:'var(--bg0)',padding:12,borderRadius:8,marginBottom:16}},
                            React.createElement('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center'}},
                                React.createElement('div',{style:{fontWeight:600,fontSize:11,cursor:'pointer'},onClick:function(){selectFile(drillDown.(data as any).path);setDrillDown(null);}},drillDown.(data as any).file),
                                React.createElement('button',{className:'view-file-btn',onClick:function(e: any){e.stopPropagation();openFilePreview(drillDown.(data as any).path,drillDown.(data as any).line);}},iconLabel('eye','View'))
                            ),
                            React.createElement('div',{style:{fontSize:10,color:'var(--t3)',marginTop:4,fontFamily:'monospace',cursor:'pointer'},onClick:function(){selectFile(drillDown.(data as any).path);setDrillDown(null);}},drillDown.(data as any).path),
                            drillDown.(data as any).line&&React.createElement('div',{style:{fontSize:10,color:'var(--orange)',marginTop:4}},'Line ',drillDown.(data as any).line)
                        ),
                        drillDown.(data as any).code&&React.createElement(React.Fragment,null,
                            React.createElement('div',{style:{fontSize:12,fontWeight:600,marginBottom:12}},'Code'),
                            React.createElement('pre',{style:{background:'var(--bg0)',padding:12,borderRadius:8,fontSize:10,fontFamily:'monospace',overflow:'auto',whiteSpace:'pre-wrap',wordBreak:'break-all'}},drillDown.(data as any).code)
                        ),
                        ...
                        React.createElement('div',{style:{background:'var(--bg0)',padding:12,borderRadius:8,fontSize:10}},
                            drillDown.(data as any).title==='Hardcoded Secret'?'Move credentials to environment variables (process.env) or a secrets manager like AWS Secrets Manager, HashiCorp Vault, or .env files (not committed to git).':
                            drillDown.(data as any).title==='SQL Injection Risk'?'Use parameterized queries or prepared statements. Never concatenate user input directly into SQL strings.':
                            drillDown.(data as any).title==='XSS Vulnerability'?'Sanitize user input before rendering. Use textContent instead of innerHTML, or use a sanitization library like DOMPurify.':
                            drillDown.(data as any).title==='Dynamic Code Execution'?'Avoid eval() entirely. Use JSON.parse() for JSON, or Function constructor only with trusted input.':
                    ...
                    drillDown.type==='duplicate'&&React.createElement(React.Fragment,null,
                        React.createElement('div',{style:{background:'var(--bg0)',padding:12,borderRadius:8,marginBottom:16}},
                            React.createElement('div',{style:{fontSize:11,color:'var(--t2)'}},drillDown.(data as any).suggestion)
                        ),
                        React.createElement('div',{style:{fontSize:12,fontWeight:600,marginBottom:12}},'All Locations (',drillDown.(data as any).files.length,')'),
                        drillDown.(data as any).files.map(function(f: any, j: any){return React.createElement('div',{key:j,style:drillDown.(data as any).type==='code'
                            ...
                            React.createElement('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center'}},
                                React.createElement('div',{style:{fontWeight:600,fontSize:11,cursor:'pointer'},onClick:function(){selectFile(f.file);setDrillDown(null);}},f.name||drillDown.(data as any).name),
                        ...
                        React.createElement('div',{style:{background:'var(--bg0)',padding:12,borderRadius:8,fontSize:10}},
                            drillDown.(data as any).type==='code'?'Extract the similar code into a shared utility function. This reduces maintenance burden and ensures consistent behavior.':
