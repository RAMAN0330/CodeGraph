...
import { detectJsPatterns, detectJsSecrets, detectDuplicatesViaTokens, isReexportBarrel, detectTreeSitterSecrets, detectPythonAstPatterns } from './analysisRules';
...
const COLORS=['#2f6fd1','#7c3fa8','#147a89','#1d7a3c','#D97706','#b8306f','#c22b3a','#5a7a0f'];
const LAYER_COLORS={ui:'#2f6fd1',components:'#147a89',services:'#7c3fa8',utils:'#1d7a3c',data:'#D97706',config:'#b8306f',test:'#92600a',modules:'#7c3fa8',forms:'#147a89',classes:'#D97706',note:'#a83e8c'};
...
const Parser={
    // Real tree-sitter AST parsers (WASM), one per language, loaded on demand
    // and cached here for the lifetime of the page. Previously Python-only and
    // fed by a `TreeSitter` browser global that nothing ever actually loaded
    // (no <script> tag, no import anywhere) — this path always silently
    // returned null client-side; only the server's runAnalysis.ts (which
    // awaits initTreeSitter('python') up front via a real npm import) ever
    // exercised it. Now backed by the same npm packages server-side already
    // used (web-tree-sitter + tree-sitter-wasms), fetching each grammar's
    // .wasm from public/wasm/ (populated by scripts/copy-tree-sitter-wasm.mjs
    // on every `npm install` — see that script for why it isn't bundled via a
    // normal import: a bundler-transformed .wasm broke Emscripten's dynamic
    // linking with a "need dylink section" error).
    _tsParsers:{},
    _tsInitPromises:{},
    initTreeSitter:async function(lang='python'){
        ...
        this._tsInitPromises[lang]=(async()=>{
            try{
                var mod=await import('web-tree-sitter');
                var TSParser=mod.Parser,TSLanguage=mod.Language,TSQuery=mod.Query;
                if(!Parser._tsCoreInitPromise){
                    Parser._tsCoreInitPromise=TSParser.init({
                        locateFile:function(){return '/wasm/tree-sitter.wasm';}
                    });
                }
                await Parser._tsCoreInitPromise;
                var Lang=await TSLanguage.load('/wasm/tree-sitter-'+lang+'.wasm');
                var parser=new TSParser();
                parser.setLanguage(Lang);
                // Query must come from this SAME import of web-tree-sitter, not a
                // separately dynamic-imported copy (e.g. analysisRules.ts importing
                // it independently) — Parser.init() only initializes the Emscripten
                // runtime state on the module instance it was called through, and a
                // Query built from a different instance throws deep inside its
                // constructor ("Cannot read properties of undefined (reading
                // 'lengthBytesUTF8')") since that instance's runtime was never set up.
    ...
    detectPatterns:async function(files){
        ...
        // (e.g. Mixins now requires an actual 2+ base class definition, not a
        // class merely named "FooMixin").
...
function highlightSyntax(code: string|null|undefined, filename: string): string[]{
    ...
    return code.split('\n').map(function(line){
        // Every injected span is parked behind a private-use placeholder so a
        // later pass can never rewrite markup an earlier one produced. Without
        // this the keyword pass rewrites `class` inside class="syn-com", which
        // breaks the tag and leaks the attribute into the rendered line.
        var stash: string[] = [];
        function keep(cls: string, text: string, style?: string){
            stash.push('<span class="'+cls+'"'+(style?' style="'+style+'"':'')+'>'+text+'</span>');
            return String.fromCharCode(0xE000+stash.length-1);
        }
        function tag(cls: string, style?: string){
            return function(match: string){ return keep(cls, match, style); };
        }
        var escaped=line.replace(/[\uE000-\uF8FF]/g,'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
        if(isJS||isJava||isPHP||isCSS)escaped=escaped.replace(/(\/\/.*$)/gm,tag('syn-com'));
        if(isPy||isRuby)escaped=escaped.replace(/(#.*$)/gm,tag('syn-com'));
        if(isHTML)escaped=escaped.replace(/(&lt;!--[\s\S]*?--&gt;)/g,tag('syn-com'));
        escaped=escaped.replace(/(&quot;[^&]*&quot;|'[^']*'|`[^`]*`)/g,tag('syn-str'));
        escaped=escaped.replace(/\b(\d+\.?\d*)\b/g,tag('syn-num'));
        if(isJS)escaped=escaped.replace(/\b(const|let|var|function|return|if|else|for|while|do|switch|case|break|continue|try|catch|finally|throw|new|class|extends|import|export|from|default|async|await|yield|typeof|instanceof|in|of|this|super|null|undefined|true|false|void|static|get|set)\b/g,tag('syn-kw'));
        if(isPy){
            escaped=escaped.replace(/\b(async|await|def|class|return|if|elif|else|for|while|try|except|finally|raise|import|from|as|with|pass|break|continue|lambda|yield|global|nonlocal|assert|True|False|None|and|or|not|in|is|del|match|case|type)\b/g,tag('syn-kw'));
            escaped=escaped.replace(/(@\w+)/g,tag('syn-fn'));
            escaped=escaped.replace(/\b(self|cls)\b/g,tag('syn-kw','opacity:0.7'));
        }
        if(isJava)escaped=escaped.replace(/\b(public|private|protected|static|final|void|class|interface|extends|implements|return|if|else|for|while|do|switch|case|break|continue|try|catch|finally|throw|new|import|package|this|super|null|true|false)\b/g,tag('syn-kw'));
        if(isRuby)escaped=escaped.replace(/\b(def|class|module|end|return|if|elsif|else|unless|case|when|for|while|until|do|begin|rescue|ensure|raise|require|include|extend|attr_accessor|attr_reader|attr_writer|true|false|nil|self)\b/g,tag('syn-kw'));
        if(isPHP)escaped=escaped.replace(/\b(function|class|return|if|else|elseif|for|foreach|while|do|switch|case|break|continue|try|catch|finally|throw|new|public|private|protected|static|const|use|namespace|extends|implements|true|false|null)\b/g,tag('syn-kw'));
        if(isVBA)escaped=escaped.replace(/\b(Public|Private|Friend|Static|Dim|Set|Let|Get|Call|Function|Sub|End Sub|End Function|Exit Sub|Exit Function|If|Then|Else|ElseIf|End If|For|To|Step|Next|Do|Loop|While|Wend|Select|Case|End Select|With|End With|On Error|Resume|GoTo|ByVal|ByRef|Optional|ParamArray|As|Type|Enum|Const|True|False|Nothing|Empty|Null|Me|Application|ThisWorkbook|Worksheets|Cells|Range|MsgBox|InputBox|Debug\.Print)\b/gi,tag('syn-kw'));
        if(isCSS)escaped=escaped.replace(/(@media|@import|@keyframes|@font-face|!important)/g,tag('syn-kw'));
        if(isHTML){escaped=escaped.replace(/(&lt;\/?)([\w-]+)/g,function(_m: string,p1: string,p2: string){return p1+keep('syn-tag',p2);});escaped=escaped.replace(/([\w-]+)(=)/g,function(_m: string,p1: string,p2: string){return keep('syn-attr',p1)+p2;});}
        escaped=escaped.replace(/\b([a-zA-Z_]\w*)\s*\(/g,function(_m: string,p1: string){return keep('syn-fn',p1)+'(';});
        if(isJS||isJava)escaped=escaped.replace(/:\s*([A-Z]\w*)/g,function(_m: string,p1: string){return ': '+keep('syn-type',p1);});
        return escaped.replace(/[\uE000-\uF8FF]/g,function(ch: string){return stash[ch.charCodeAt(0)-0xE000]||ch;});
    });
}
...
function getDialogTone(tone){
    if(tone==='danger')return{
        color:'var(--red)',
        borderColor:'rgba(194,43,58,0.34)',
        background:'rgba(194,43,58,0.12)'
    };
    if(tone==='info')return{
        color:'var(--blue)',
        borderColor:'rgba(47,111,209,0.34)',
        background:'rgba(47,111,209,0.12)'
    };
    return{
        color:'var(--orange)',
        borderColor:'rgba(146,96,10,0.34)',
        background:'rgba(146,96,10,0.12)'
    };
}
