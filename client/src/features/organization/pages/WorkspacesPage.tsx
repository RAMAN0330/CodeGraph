...
import { ArrowRight, Building2, CheckCircle2, GitBranch, Plus, Trash2 } from 'lucide-react';
...
export default function WorkspacesPage() {
  ...
  return <main className="organization-page workspace-details-page">
    ...
    {!workspaces.length ? <section className="workspace-empty-page"><div className="empty-state-card"><span><Building2 size={34} /></span><h1>Start your first workspace</h1><p>Create a focused home for your projects and repository analysis.</p><Button className="projects-primary-action" onClick={() => dialog.current?.showModal()}><Plus size={16} /> Create workspace</Button><ul className="empty-state-points"><li><CheckCircle2 size={15} /> Group repositories by team or initiative</li><li><CheckCircle2 size={15} /> Track architecture, ownership, and risk in one place</li><li><CheckCircle2 size={15} /> Invite teammates once you're set up</li></ul></div></section> : <section className="workspace-catalog"><div className="workspace-catalog-heading"><div><h1>Workspaces</h1><p>Select a workspace to view its projects and connected repositories.</p></div><Button className="workspace-create-button" aria-label="Create workspace" onClick={() => dialog.current?.showModal()}><Plus size={18} /><span>Create workspace</span></Button></div><div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 8 }} aria-label="Workspace list">{visible.map((workspace, index) => <motion.div key={workspace.id} className="project-list-row" style={{ width: '100%' }} initial={reduceMotion ? false : { opacity: 0, y: 10 }} animate={reduceMotion ? undefined : { opacity: 1, y: 0 }} transition={{ duration: .28, delay: index * .04 }}>
