...
import { Parser, shouldExcludeFile, shouldIgnoreDirectory } from './parser';
...
var GitHub: any = {
    ...
    request(url: string, options?: any, errorMap?: any) {
        var self = this;
    ...
    getFile(o: string, r: string, p: string, branch?: string) {
        ...
        return this.fetch(url).then(function (d: any) {
            ...
            // Fallback: raw.githubusercontent.com (public repos, no rate limits)
            var br = branch || 'main';
            var rawUrl = 'https://raw.githubusercontent.com/' + o + '/' + r + '/' + br + '/' + p;
            return fetch(rawUrl).then(function (res) {
                if (res.ok) return res.text();
                if (br === 'main') {
                    return fetch('https://raw.githubusercontent.com/' + o + '/' + r + '/master/' + p)
