...
export function extractManifestDeps(files: any[]): ManifestPackage[] {
  const results: ManifestPackage[] = [];

  for (const f of files) {
    if (!f.content || !f.name) continue;

    // package.json → npm (production deps only)
    if (f.name === 'package.json') {
      try {
        const pkg = JSON.parse(f.content);
        for (const [name, ver] of Object.entries(pkg.dependencies ?? {})) {
          const v = String(ver).replace(/^[^0-9]*/, '');
          if (v) results.push({ name, version: v, ecosystem: 'npm' });
        }
    ...
    if (f.name === 'requirements.txt') {
      for (const line of f.content.split('\n')) {
        ...
        if (!clean) continue;
        const m = clean.match(/^([A-Za-z0-9_.-]+)[=~<>!]+([0-9][^\s;]*)/);
        if (m) results.push({ name: m[1], version: m[2], ecosystem: 'PyPI' });
      }
    }
    ...
    if (f.name === 'go.mod') {
      for (const line of f.content.split('\n')) {
        const m = line.trim().match(/^([^\s]+)\s+v([0-9][^\s]*)/);
        if (m && !line.trim().startsWith('module') && !line.trim().startsWith('go ')) {
    ...
    if (f.name === 'Gemfile.lock') {
      ...
      for (const line of f.content.split('\n')) {
        ...
        if (inSpecs && line.match(/^    \S/)) {
          const m = line.trim().match(/^([A-Za-z0-9_-]+)\s+\(([0-9][^)]*)\)/);
          if (m) results.push({ name: m[1], version: m[2], ecosystem: 'RubyGems' });
        }
        if (inSpecs && line.trim() === '') inSpecs = false;
      }
    }
  }
